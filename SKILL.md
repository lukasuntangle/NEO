...
- **`/neo dashboard`**: Relaunch the Matrix Dashboard if it was accidentally closed. Runs `bash scripts/launch-dashboard.sh <project-dir>`. Kills any existing dashboard process for this session before opening a new one. Detects the current terminal app (iTerm2, Warp, Ghostty, Terminal.app) and opens a new tab.
